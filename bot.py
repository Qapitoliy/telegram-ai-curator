...
TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN")  # Твой Telegram бот токен
GROQ_API_KEY = os.getenv("GROQ_API_KEY")      # Ключ Groq API
GROQ_MODEL = "llama-3.3-70b-versatile"        # Модель Groq

bot = Bot(token=TELEGRAM_TOKEN)
# Обязательно установить текущий бот, чтобы избежать ошибки RuntimeError
Bot.set_current(bot)
...
async def query_groq_api(prompt: str) -> str:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=json_data) as resp:
            ...
            # Примерная структура ответа Groq (уточни в их доках!)
            # Ответ может быть в data['choices'][0]['text']
            try:
